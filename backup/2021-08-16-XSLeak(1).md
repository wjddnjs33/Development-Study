...
5. endsWith 값을 가져와서 str 변수에 저장을 합니다.
...
여기서 중요한 부분은 우리가 입력한 값과 노트의 값이 일치 하지 않으면 else 문 내부에 들어가는데, 여기서 응답 헤더를 하나를 우리가 설정할 수 있다는 것 입니다. 우린 아까 위에서 `Download Trigger`에 대해서 배웠는데, 엔드포인트에서 `Content-Disposition: attachment` 헤더를 설정해서 반환하면 브라우저는 첨부 파일을 탐색하고 파일을 설치 한다는 것을 배웠습니다.
